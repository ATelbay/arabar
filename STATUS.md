...
|---|---|---|
| Стек | Swift + SwiftUI `MenuBarExtra` | Нативный, маленький бинарь, поддержка macOS 14+ |
...
- **[P0] Тщательное ревью всего проекта первым делом**. До любых новых фич. Проект пережил 10+ итераций с параллельными агентами, накопился drift. Чек-лист:
  - **Дубликаты** — повторяющиеся helpers (profile enumeration, SQLite open, AES decrypt, debugLog) в разных файлах. Особенно подозрительны: `ChromiumCookieDB.swift` vs `OpenAICookiesReader.swift`/`ClaudeCookiesReader.swift` (DB lock copy-to-temp паттерн), Chrome Safe Storage decryption (был баг с `"Chrome Safe Storage Safe Storage"` — точно ли исправлен везде).
  - **Мёртвый код** — пройтись по `grep -r "private func"` и проверить usage. Уже всплыл `applyHeaders` (T31). Возможно есть ещё.
  - **Архитектурные несогласованности** — типа того что было в T29 (subscription был либо-либо, должно быть merge). Проверить аналогичные места:
    - Admin API path vs JSONL для одного провайдера — мержим ли token counts? (Сейчас отдельные snapshot — может стоит мержить или явно показывать "почему два"?)
    - `Aggregator` per-provider vs `[Provider: UsageSnapshot]` — single source of truth?
    - `eventBuffer` в `AppViewModel` vs cookies-only пользователь — буфер бесполезен но пишется на диск, нужно ли гейтить?
  - **Error handling consistency** — каждый ридер кидает свой enum, UI местами показывает `.localizedDescription`, местами кастомные мапы. Унифицировать?
  - **Security pass** — Keychain доступ, cookies дешифровка, никогда не логируем secrets (T28 гейтил debug, но проверить что в release пути нет утечек). Безопасность `Process` shell out на `/usr/bin/security` (T-? Bug-D fix) — нет ли command injection если username имеет странные chars?
  - **Test coverage** — сейчас по сути ноль unit-тестов. Хотя бы для `Aggregator.aggregate()`, `ChromiumCookieDB.cookieExpiry()`, `Pricing.cost(model:tokens:)` — критичные pure-функции.
  - **`PlanLimits.swift`** — комьюнити-оценки помечены `// TODO: уточнить` ещё с фазы 2. Если cookies path работает у большинства — может удалить весь fallback и просто показывать `ukwn` для JSONL-only пользователей вместо ложного %.
  - **Memory / Refresh leaks** — Timer в `Scheduler.swift` и `MenuBarLabel.swift` имеют `[weak self]`? `Task.detached` в `refresh()` (T30+T32 добавили) — не утекают ли?
  - **Подход**: можно через ultrareview (мульти-агентное cloud-ревью). Либо последовательно review-current + security-review skills. По итогу — список конкретных fix tasks, потом запустить parallel agents.

### Остаточный бэклог

Только future-major work: Sparkle auto-update, WidgetKit, поддержка остальных 25+ провайдеров (CodexBar list). MVP feature-complete, регрессий нет.
